...
author: "Javier E. Flores, Anastasiya V. Prymolenna, Natalie M. Winans, Logan A. Lewis, Allison Thompson, William R. Kew, Robert P. Young, Lisa M. Bramer"
...
In the "Experiment Metadata" tab, enter your proposal number and PI Name, or select from the dropdown if you have entered these before. These will be saved to a database as a profile, in order to save settings and selections between sessions. Select a project name from the dropdown to make edits to a previous saved session or click the text box to name a new session.
...
Progress to the next step by clicking the "Metabolite Identification" button.
...
* **To load from a previous save**, click the "Choose a previous session:" box to show a drop down of the previous sessions tied to your user profile. Select the one you wish to load and click load previous session. The metabolites you selected in that session and the edits you made to those metabolites will populate on the right of the screen.
...
You can choose metabolite you wish to profile using [*rDolphin's Baseline Sum profiling function*](https://github.com/danielcanueto/rDolphin/blob/master/R/integration.R) instead of the default Baseline Fitting. Profiling with Baseline Sum does not return a Fitting Error.
...
The "Signal View" tab is empty until profiling is performed, at which time it will be populated with final results and visualizations.
...
The signal view tab contains 6 ggplot displays of metabolite peaks. These plots consist of signal-by-signal fits of a signal to particular spectrum region.

Filtering and sorting can be performed on the basis of sample, signal, and fitting error using the options below the plots and then clicking "Apply Filter". 

```{r, echo = F, out.width='120%'}
include_graphics("trelliscope.gif")
```

The individual plots in the display each show four lines:
