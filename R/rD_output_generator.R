...
#'
#' @details This function is based on code that was forked directly from rDolphin (https://github.com/danielcanueto/rDolphin/tree/master/R). This function formats
#' the output generated by the fitting routine and computes error metrics based on this output.
#'
output_generator <- function(signals_to_quantify, fitted_signals, Ydata,
                             Xdata, signals_parameters, multiplicities, buck_step){
  ...
  output_data$chemical_shift <- as.numeric(signals_parameters[2, !BGsignals])
  output_data$intensity      <- as.numeric(signals_parameters[1, !BGsignals])
  output_data$half_bandwidth <- as.numeric(signals_parameters[3, !BGsignals ])
  dummy                      <- list(output_data = output_data,
                                     error1      = error1)
  return(dummy)
