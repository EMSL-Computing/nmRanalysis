#' rDolphin: integration
#'
#' @param clean_fit Either "Y" or "N".
...
#' @param interface Either "T" or "F". Indicates whether data for plotting should be generated.
#'
#' @details This function is based on code that was forked directly from rDolphin (https://github.com/danielcanueto/rDolphin/tree/master/R). This function performs quantification through integration.
#'
integration <- function(clean_fit, Xdata, Ydata, buck_step, interface='F'){

  #preallocation of results_to_save
  results_to_save <- list(chemical_shift    = NA,
                          quantification    = NA,
  ...
  if (clean_fit == 'N'){
    baseline <- seq(min(Ydata[1:5]), min(Ydata[(length(Xdata) - 4):length(Xdata)]), len = length(Xdata))
  }
  ...
  results_to_save$signal_area_ratio <- tryCatch((sum(integrated_signal[p1:p2]) / sum(Ydata[p1:p2])) * 100,
                                                error = function(e){NaN}, silent=T)
  ...
  if(results_to_save$chemical_shift == Inf){
    results_to_save$chemical_shift <- mean(Xdata)
  }
  ...
  if (interface == 'T') {
    ...
    p                  <- plotly::plot_ly(plotdata,x = ~Xdata, y = ~signal, type = 'scatter', color= 'Signal',mode = 'lines', fill = 'tozeroy') %>%
                          plotly::add_trace(data = plotdata3, x = ~Xdata, y = ~value, color = ~variable,type = 'scatter', mode = 'lines', fill = NULL) %>%
                          plotly::layout(xaxis = list(range = c(Xdata[1], Xdata[length(Xdata)]), title = 'ppm'),
  ...
  integration_variables <- list(results_to_save = results_to_save,
                                p               = p,
                                plot_data       = plot_data)
