...
#'
#' @details This function is based on code that was forked directly from rDolphin (https://github.com/danielcanueto/rDolphin/tree/master/R). This function is used to collate
...
fitting_prep <- function(Xdata, Ydata, initial_fit_parameters, program_parameters, created_baseline){
  ...
  colnames(initial_fit_parameters) = c("quantification_or_not",
                                       "positions",
                                       "shift_tolerance",
                                       "widths",
                                       "multiplicities",
  ...
  BGSigNum <- ifelse(program_parameters$clean_fit == 'N',
                     max(round(abs(Xdata[1] - Xdata[ROIlength]) * program_parameters$BGdensity), 3), 0)

  #Preallocation of parameters to optimize into a matrix of features
  FeaturesMatrix <- matrix(NA, (signals_to_fit + BGSigNum), 15)

  # modified to allow for character-valued multiplicities and Jcoupling2 and roof effect 2
  FeaturesMatrix <- as.data.frame(FeaturesMatrix)

  colnames(FeaturesMatrix) <- c('minimum_intensity',
                                'maximum_intensity',
                                'shift_left_limit',
                                'shift_right_limit',
                                'minimum_width',
  ...
  FeaturesMatrix[1:signals_to_fit, 9]  <- initial_fit_parameters$Jcoupling - program_parameters$j_coupling_variation
  FeaturesMatrix[1:signals_to_fit, 10] <- initial_fit_parameters$Jcoupling + program_parameters$j_coupling_variation
  ...
  # modified to accommodate second J coupling value and second roof effect (for dd only)
  FeaturesMatrix[1:signals_to_fit, 13] <- initial_fit_parameters$Jcoupling2 - program_parameters$j_coupling_variation
  FeaturesMatrix[1:signals_to_fit, 14] <- initial_fit_parameters$Jcoupling2 + program_parameters$j_coupling_variation
  ...
  if (BGSigNum > 0) {
    BGSigrightlimits <- seq(Xdata[1] - 0.005, Xdata[ROIlength] + 0.005, length = BGSigNum) - 0.005
