...
#'
#' @details This function was forked directly from rDolphin (https://github.com/danielcanueto/rDolphin/tree/master/R). This function is responsible
...
fittingloop_bg <- function(FeaturesMatrix, Xdata, Ydata, program_parameters){
  ...
  # 6/7/22 modification by Javier E. Flores to allow for additional J coupling value and roof effect value (for dd case)
  # Note that the additional J coupling value is baked into par (parS) so that it may be optimized along with the other
  # j coupling value.
  residFun <-
    function(par, observed, xx, multiplicities, roof_effect, roof_effect2, freq)
  ...
  lb = as.vector(t(FeaturesMatrix[, c(seq(1, 9, 2), 13), drop = F])) # 6/8/22 modification by Javier E. Flores to allow for additional J coupling value (for dd case)
  ub = as.vector(t(FeaturesMatrix[, c(seq(2, 10, 2), 14), drop = F])) # 6/8/22 modification by Javier E. Flores to allow for additional J coupling value (for dd case)
  ...
  nls.out <-
    minpack.lm::nls.lm(
      par            = s0, # 6/7/22 modification by Javier E. Flores to allow for additional J coupling value (for dd case)
      fn             = residFun,
      observed       = Ydata,
      xx             = Xdata,
      multiplicities = multiplicities,
  ...
  # 6/8/22 modification by Javier E. Flores to allow for additional J coupling value (for dd case)
