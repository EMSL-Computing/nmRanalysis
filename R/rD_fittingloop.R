...
fittingloop <- function(FeaturesMatrix, Xdata, Ydata, program_parameters){
  ...
  # 6/8/2022 Modification by Javier E. Flores to allow for second J-coupling value in optimization
  ...
  paramprov = rep(0, nrow(FeaturesMatrix)*6) # 6/8/2022 Modification by Javier E. Flores to allow for second J-coupling value in optimization
  ...
  # 6/7/22 modification by Javier E. Flores to allow for additional J coupling value and roof effect (for dd case)
  # Note that the additional J coupling value is baked into par (parS) so that it may be optimized along with the other
  # j coupling value.
  residFun <-
    function(par, observed, xx, multiplicities, roof_effect, roof_effect2, freq, bins)
  ...
  while (iterrep <= fitting_maxiterrep) {
    ...
    # 6/7/22 modification by Javier E. Flores to allow for additional J coupling value (for dd case)
    ...
    for (i in signals_to_fit) {
      ...
      if(length(sorted_bins$x) > 0) {
        # 6/8/22 Note by Javier E. Flores. This section may need to be edited to accommodate second J-coupling value
        # since the quantities seem to depend on the existing J coupling value. However, I do not understand what this
        # code chunk does so I am not sure whether (or what) modifications may be necessary.
        bins2 = sorted_bins$ix[1:which.min(abs(cumsum(sorted_bins$x)-0.9))]
        # 6/6/22 modification by Javier E. Flores to adapt to FeaturesMatrix as a dataframe
    ...
    while (error1 > program_parameters$errorprov &error1 > (1 / 3 * worsterror) & iter < fitting_maxiter) {
      ...
      # 6/8/22 modification by Javier E. Flores to accommodate optimization of second J-coupling value.
      ...
      if (iter<4&length(peaks_xdata$maxtab$val)>0) {
        ...
        # 6/6/22 modification by Javier E. Flores to adapt to multiplicity as character-valued
        ...
        set.seed(iter);nls.out <-
          minpack.lm::nls.lm(
            par            = s0, # 6/7/22 modification by Javier E. Flores to allow for additional J coupling value and roof effect (for dd case)
            fn             = residFun,
            observed       = Ydata,
            xx             = Xdata,
            multiplicities = multiplicities,
        ...
        iter = iter + 1
        # 6/7/22 modification by Javier E. Flores to account for additional J coupling value and roof effect (for dd case)
        ...
        # 6/8/22 modification by Javier E. Flores to account for second J coupling value
        order2=order(coef(nls.out)[which(seq_along(coef(nls.out))%%6==2)][signals_to_fit])
        # #Procedure to calculate the fititng error in all the ROI
        #An adapted MSE error is calculated, and the parameters of the optimization with less MSE are stored
        errorprov = (sqrt(nls.out$deviance / length(Ydata))) * 100 / (max(Ydata) - min(Ydata))
    ...
    # 6/8/22 modification by Javier E. Flores to allow for second J coupling value
    ...
    # 6/8/22 modification by Javier E. Flores to allow for J coupling 2 optimization
    ...
    if (iterrep <= fitting_maxiterrep& error22 < (program_parameters$additional_signal_improvement * dummy) &
      ...
      if (is.null(residual_peaks$maxtab) == F) {
        ...
        dummy[dummy==0]=2
        # 6/8/22 modification by Javier E. Flores to account for j coupling 2
